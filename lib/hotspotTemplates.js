...
export function generateFactoryDefaultTemplate(opts = {}) {
  const businessName = opts.businessName || opts.wifiSsid || 'Internet Hotspot';
  return `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
<title>${escapeHtml(businessName)} - Log in</title>
<style>
body {
...
<div class="box">
  ...
  <form name="login" action="$(link-login-only)" method="post" onsubmit="if(!this.password.value&&this.username.value){this.password.value=this.username.value.trim();}">
    ...
    <table>
      <tr>
        <td class="label">User / Voucher:</td>
        <td><input type="text" name="username" value="$(username)" autofocus autocomplete="off" /></td>
      </tr>
      <tr>
