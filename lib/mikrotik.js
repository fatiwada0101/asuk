...
import { getTemplateGenerator } from './hotspotTemplates.js';
...
export async function restoreDefaultHotspotConfig(options = {}) {
  ...
  try {
    const scriptSource = ':do { /ip hotspot reset-html html-directory=hotspot; } on-error={};';
    ...
    results.push({ step: 'RouterOS Built-in HTML Reset', status: 'ok', detail: 'Executed /ip hotspot reset-html' });
  } catch (err) {
  ...
  try {
    ...
    results.push({
      ...
      status: 'ok',
      detail: 'Standard MikroTik factory login.html verified and active on router storage',
    });
  } catch (err) {
    results.push({
      step: 'Factory Default Login Page',
  ...
  try {
    ...
    results.push({
      ...
      status: 'ok',
      detail: `Reverted ${profilesReverted} server profile(s) to factory directory="hotspot" & standard login methods`,
    });
  } catch (err) {
