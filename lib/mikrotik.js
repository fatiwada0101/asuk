...
/**
 * Generate a standalone, responsive, modern dark-mode Hotspot login.html page
 * embedded with the buy voucher link pointing directly to www.asuk.tech
 */
export function generateHotspotLoginHtml(options = {}) {
  const wifiSsid = options.wifiSsid || 'Asuk Tech Wi-Fi';
  const buyUrl = options.buyUrl || 'https://www.asuk.tech/packages';
...
.card{width:100%;max-width:390px;background:#18181b;border:1px solid #27272a;border-radius:22px;padding:32px 24px;box-shadow:0 24px 48px rgba(0,0,0,0.6);text-align:center}
.icon{width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg,#7257ff,#5438dc);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;font-size:28px;box-shadow:0 8px 24px rgba(114,87,255,0.35)}
h1{font-size:22px;font-weight:800;margin-bottom:6px;color:#ffffff}
p.sub{font-size:13px;color:#a1a1aa;margin-bottom:24px}
.err{background:rgba(239,68,68,0.15);border:1px solid rgba(239,68,68,0.4);color:#f87171;padding:10px 14px;border-radius:12px;font-size:13px;margin-bottom:18px;line-height:1.4}
.lbl{display:block;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#a1a1aa;margin-bottom:8px;text-align:left}
input[type="text"]{width:100%;padding:14px 16px;font-size:18px;font-weight:800;background:#27272a;border:1.5px solid #3f3f46;border-radius:14px;color:#fff;text-align:center;letter-spacing:2px;outline:none;font-family:monospace;text-transform:uppercase;transition:border-color .2s}
input[type="text"]:focus{border-color:#7257ff}
.btn{width:100%;padding:15px;margin-top:18px;font-size:15px;font-weight:800;background:linear-gradient(135deg,#7257ff,#5438dc);color:#fff;border:none;border-radius:14px;cursor:pointer;box-shadow:0 4px 20px rgba(114,87,255,0.4);transition:transform .1s,box-shadow .2s}
...
.btn-buy{display:inline-flex;align-items:center;justify-content:center;gap:6px;color:#c4b5fd;font-size:13.5px;font-weight:700;text-decoration:none;padding:11px 20px;background:rgba(114,87,255,0.12);border:1px solid rgba(114,87,255,0.3);border-radius:12px;transition:background .2s,border-color .2s}
.btn-buy:hover{background:rgba(114,87,255,0.2);border-color:#7257ff}
...
<a href="${buyUrl}?mac=$(mac)&ip=$(ip)&link-login-only=$(link-login-only-esc)&link-orig=$(link-orig-esc)" class="btn-buy">&#128722; Buy a Data Plan Online (www.asuk.tech) &rarr;</a>
</div>
...
/**
 * Push the generated Hotspot login.html directly to MikroTik router storage
 */
export async function pushHotspotLoginPageToRouter(options = {}) {
  ...
  const wifiSsid = options.wifiSsid || config.wifi_ssid || 'Asuk Tech Wi-Fi';
  const buyUrl = options.buyUrl || 'https://www.asuk.tech/packages';
  const portalHtml = generateHotspotLoginHtml({ wifiSsid, buyUrl });

  // Update hotspot server profiles to use html-directory="hotspot"
  try {
  ...
  // Method A: Direct REST API file update
  try {
    const { url: fileListUrl, headers: fileHeaders } = await buildMikroTikRequest('/rest/file');
    const fileRes = await fetch(fileListUrl, { headers: fileHeaders });
    if (fileRes.ok) {
      const files = await fileRes.json();
      const loginFile = Array.isArray(files) && files.find(f => f.name === 'hotspot/login.html' || f.name === 'login.html');
      if (loginFile && loginFile['.id']) {
        const { url: patchUrl, headers: patchHeaders } = await buildMikroTikRequest(`/rest/file/${encodeURIComponent(loginFile['.id'])}`);
        const patchRes = await fetch(patchUrl, {
          method: 'PATCH',
          headers: patchHeaders,
          body: JSON.stringify({ contents: portalHtml }),
        });
        if (patchRes.ok) portalInstalled = true;
      }
    }
  } catch {}
  ...
  if (!portalInstalled) {
    ...
    try {
      const { url: scriptListUrl, headers: scriptHeaders } = await buildMikroTikRequest('/rest/system/script');
      const sListRes = await fetch(scriptListUrl, { headers: scriptHeaders });
      ...
      if (existingScript) {
        ...
        await fetch(setUrl, { method: 'POST', headers: setH, body: JSON.stringify({ '.id': existingScript['.id'], source: scriptSource }) });
      } else {
        const { url: addUrl, headers: addH } = await buildMikroTikRequest('/rest/system/script/add');
        await fetch(addUrl, { method: 'POST', headers: addH, body: JSON.stringify({ name: scriptName, source: scriptSource, policy: 'ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon' }) });
      }

      const { url: runUrl, headers: runH } = await buildMikroTikRequest('/rest/system/script/run');
      await fetch(runUrl, { method: 'POST', headers: runH, body: JSON.stringify({ number: scriptName }) });
      portalInstalled = true;
    } catch (err) {
      throw new Error('Failed to install portal HTML: ' + err.message);
    }
  }

  return { success: true, portalInstalled, wifiSsid, buyUrl };
}
