...
export async function getMikroTikConfig() {
  try {
    ...
    if (!error && data?.value && data.value.ip) {
      ...
      return {
        ...
        protocol,
        isConfigured,
        hotspot_url: cfg.hotspot_url ? cfg.hotspot_url.trim() : 'asuktech.net',
        wifi_ssid: cfg.wifi_ssid ? cfg.wifi_ssid.trim() : 'Asuk Tech Wi-Fi',
      };
    }
  } catch (err) {
  ...
  if (process.env.MIKROTIK_IP && process.env.MIKROTIK_IP.trim()) {
    ...
    return {
      ...
      protocol,
      isConfigured,
      hotspot_url: process.env.MIKROTIK_HOTSPOT_URL || 'asuktech.net',
      wifi_ssid: process.env.MIKROTIK_WIFI_SSID || 'Asuk Tech Wi-Fi',
    };
  }
  ...
  if (!config.isConfigured) {
    ...
    remediation = [
      ...
      isPrivate
        ...
        : `Check router WAN IP and ensure port ${config.port} is forwarded on your upstream modem/firewall.`,
      ...
      `In WinBox, check "IP" -> "Firewall" -> "Filter Rules" to ensure no rule is dropping incoming traffic on port ${config.port}.`
    ];
  } else if (isTlsError) {
    errorCode = 'TLS_ERROR';
...
export async function testConnection(options = {}) {
  ...
  const target = `${config.protocol}://${config.ip}:${config.port}`;
