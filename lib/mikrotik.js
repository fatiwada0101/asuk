...
export async function getMikroTikConfig() {
  ...
  const ip = (process.env.MIKROTIK_IP || '192.168.88.1').trim();
  ...
  return { ip, user, pass, port: String(port), protocol };
}
 ...
 * Throws error if router is unreachable or creation fails.
 *
 * @param {Object} opts
 ...
 */
export async function createHotspotUser({ code, password, profile, limitUptime, comment, shared_users, rate_limit }) {
  ...
  try {
    ...
    return {
      ...
      name: code,
      profile: bodyData.profile || 'default',
    };
  } catch (err) {
    if (err.name === 'AbortError') {
