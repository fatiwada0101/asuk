...
export async function createHotspotUser({ code, password, profile, limitUptime, comment, shared_users, rate_limit, expiry_mode, limit_bytes_total }) {
  ...
  // Core user body — only universally supported parameters
  const bodyData = {
    name: code,
  ...
  try {
    ...
    // If profile not found or unknown parameter, retry with simplified body
    if (!res.ok) {
      const errText = await res.text();
      const errLower = errText.toLowerCase();

      if (res.status === 400 && (errLower.includes('profile') || errLower.includes('unknown parameter'))) {
        console.warn(`Router rejected request (400): "${errText.substring(0, 120)}". Retrying with simplified body...`);

        // Strip profile and any optional fields for retry
        const retryBody = {
          name: bodyData.name,
          password: bodyData.password,
          comment: bodyData.comment,
        };
        if (bodyData['limit-uptime']) retryBody['limit-uptime'] = bodyData['limit-uptime'];
        if (bodyData['limit-bytes-total']) retryBody['limit-bytes-total'] = bodyData['limit-bytes-total'];
        ...
        if (retryRes.ok) {
          ...
          const usedProfile = 'default';

          // Apply shared-users and rate-limit to the PROFILE instead (fire-and-forget)
          applyProfileSettings(bodyData.profile || usedProfile, { shared_users, rate_limit }).catch(() => {});
          applyExpiryModeToProfile(usedProfile, limitUptime, expiry_mode).catch(() => {});
        ...
        throw new Error(`MikroTik router error (${retryRes.status}): ${retryErrText || errText}`);
      }
    ...
    // Apply shared-users and rate-limit to the PROFILE (fire-and-forget, non-blocking)
    applyProfileSettings(usedProfile, { shared_users, rate_limit }).catch(() => {});
...
/**
 * Apply shared-users and rate-limit to a hotspot user profile.
 * These are profile-level settings in MikroTik, not user-level.
 */
async function applyProfileSettings(profileName, { shared_users, rate_limit } = {}) {
  if (!profileName) return;
  const hasShared = shared_users && Number(shared_users) > 1;
  const hasRate = rate_limit && rate_limit !== '/' && rate_limit !== 'undefined/undefined';
  if (!hasShared && !hasRate) return;

  try {
    const { url: profileUrl, headers } = await buildMikroTikRequest('/rest/ip/hotspot/user/profile');
    const profilesRes = await fetch(profileUrl, { headers, signal: AbortSignal.timeout(5000) });
    ...
    const patchData = {};
    if (hasShared) patchData['shared-users'] = String(shared_users);
    if (hasRate) patchData['rate-limit'] = rate_limit;

    await fetch(`${profileUrl}/${target['.id']}`, {
      method: 'PATCH',
      headers,
    ...
    console.warn(`Non-fatal: could not apply profile settings to "${profileName}":`, e.message);
  }
}
...
export async function setHotspotDnsName(dnsName) {
  ...
  for (const prof of profiles) {
    if (!prof['.id']) continue;
    try {
      // Attempt A: POST /rest/ip/hotspot/profile/set (works on most RouterOS 7 firmware)
      ...
      if (setRes.ok) {
        ...
        // Attempt B: PATCH /rest/ip/hotspot/profile/{id} (fallback for firmware variants that reject POST /set)
        const profId = encodeURIComponent(prof['.id']);
        const { url: patchUrl, headers: patchH } = await buildMikroTikRequest(`/rest/ip/hotspot/profile/${profId}`);
...
// Re-export companion template generators for consumers importing from mikrotik.js
export {
  generateHotspotAloginHtml,
  generateHotspotStatusHtml,
  generateHotspotLogoutHtml,
  ...
  try {
    ...
    if (wgRes.ok) {
      ...
      for (const e of (Array.isArray(entries) ? entries : [])) {
        ...
        const srv = (e['server'] || e['src-address'] || '').toLowerCase();
        if (h.includes('asuktech.net') || h.includes(hotspotUrl.toLowerCase()) || h.includes('10.5.50.1') || srv.includes('10.5.50.1')) {
          try {
...
export async function restoreDefaultHotspotConfig(options = {}) {
  ...
  // Fetch full config — needed for hotspot_url when reverting server profiles
  const config = await getMikroTikConfig();
  const results = [];
