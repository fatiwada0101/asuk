...
export async function createHotspotUser({ code, password, profile, limitUptime, comment, shared_users, rate_limit, expiry_mode, limit_bytes_total }) {
  ...
  // Set shared users (how many simultaneous sessions this voucher allows)
  if (shared_users && Number(shared_users) > 1) {
    bodyData['shared-users'] = String(shared_users);
  }

  // Set per-user rate limit (upload/download speed)
  if (rate_limit && rate_limit !== '/' && rate_limit !== 'undefined/undefined') {
