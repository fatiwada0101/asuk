...
// Helper to format currency for canvas/HTML
...
// Helper to format currency for PDF text (ASCII-safe to avoid standard font symbol corruption)
export function formatPdfCurrency(amount) {
  const num = Number(amount || 0);
...
/**
 * Truncates text with an ellipsis if it exceeds maxWidth on a 2D canvas
 */
...
export function generatePassReceiptCanvas(passData = {}) {
  ...
  // Normalized pass variables
  const brandName = passData.brandName || 'ASUK TECH WI-FI';
  const planName = passData.planName || passData.plan_name || passData.plan || passData.profile_name || 'Wi-Fi Access Pass';
  const duration = passData.duration || (passData.limit_uptime_seconds ? `${Math.round(passData.limit_uptime_seconds / 3600)} Hours` : 'Standard Session');
  const voucherCode = passData.voucherCode || passData.voucher_code || passData.code || '00000';
  const price = passData.price || passData.amount || 0;
  const paymentMethod = passData.paymentMethod || passData.payment_method || 'Online Payment';
  const wifiSsid = passData.wifiSsid || passData.wifi_ssid || 'Asuk Tech Wi-Fi';
  const hotspotUrl = passData.hotspotUrl || passData.hotspot_url || 'asuktech.net';
  const createdAt = passData.createdAt || passData.created_at;
  const isUsed = passData.isUsed !== undefined ? passData.isUsed : (passData.is_used || false);

  // Branding Title
  ctx.fillStyle = '#FFFFFF';
  ctx.font = '800 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
  ctx.textAlign = 'center';
  ctx.fillText(brandName, w / 2, margin + 42);
  ...
  ctx.fillStyle = isUsed ? '#334155' : '#10B981';
  roundRect(ctx, pillX, pillY, pillW, pillH, 12);
  ctx.fill();

  ctx.fillStyle = '#FFFFFF';
  ctx.font = '700 11px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
  ctx.fillText(isUsed ? '● PASS USED' : '● READY TO CONNECT', w / 2, pillY + 16);

  // Timestamp & Ref
  ctx.fillStyle = '#64748B';
  ctx.font = '500 11px monospace';
  ctx.fillText(formatReceiptDate(createdAt), w / 2, margin + 130);
  ctx.restore();
  ...
  ctx.fillText(voucherCode, w / 2, curY + 62);
  ctx.restore();
  ...
  const maxValW = cardW - 190;

  const rows = [
    { label: 'Hotspot Plan', value: planName },
    { label: 'Validity / Duration', value: duration },
    { label: 'Amount Paid', value: formatCurrency(price) },
    { label: 'Payment Method', value: paymentMethod },
    { label: 'Network Name (SSID)', value: wifiSsid },
    { label: 'Login Portal', value: hotspotUrl },
  ...
  rows.forEach((row, idx) => {
    ...
    ctx.textAlign = 'right';
    const safeVal = truncateText(ctx, row.value, maxValW);
    ctx.fillText(safeVal, rightX, curY);
  ...
  drawBarcode(ctx, barcodeX, curY, barcodeW, barcodeH);

  ctx.fillStyle = '#71717A';
  ctx.font = '500 10.5px monospace';
  ctx.textAlign = 'center';
  ctx.fillText(`* ${passData.voucherCode || 'PIN'} *`, w / 2, curY + barcodeH + 16);
...
export function generateTransactionReceiptCanvas(txData = {}) {
  ...
  // Normalized transaction variables
  const brandName = txData.brandName || 'ASUK TECH WI-FI';
  const isDeposit = txData.type === 'wallet_topup' || txData.type === 'deposit';
  const amount = txData.amount || 0;
  const createdAt = txData.createdAt || txData.created_at;
  const flwRef = txData.flwRef || txData.flw_ref;
  const paymentMethod = txData.paymentMethod || txData.payment_method || (isDeposit ? 'Card / Bank Transfer' : 'Wallet Balance');
  const userEmail = txData.userEmail || txData.user_email || txData.email;
  ...
  ctx.textAlign = 'center';
  ctx.fillText(brandName, w / 2, margin + 40);
  ...
  ctx.fillText(`${prefix}${formatCurrency(amount)}`, w / 2, margin + 108);
  ...
  const rowH = 30;
  const maxValW = cardW - 180;

  const rows = [
    { label: 'Transaction Type', value: isDeposit ? 'Wallet Deposit' : 'Wi-Fi Pass Purchase' },
    { label: 'Date & Time', value: formatReceiptDate(createdAt) },
    { label: 'Payment Gateway Ref', value: flwRef ? `${flwRef.substring(0, 18)}…` : 'Internal Wallet' },
    { label: 'Payment Method', value: paymentMethod },
    { label: 'Transaction Status', value: 'Completed' },
  ];

  if (!isDeposit && metadata?.plan_name) {
    rows.unshift({ label: 'Wi-Fi Plan', value: metadata.plan_name });
  }
  if (!isDeposit && metadata?.voucher_code) {
    rows.unshift({ label: 'Voucher PIN', value: metadata.voucher_code });
  }
  if (userEmail) {
    rows.push({ label: 'Account Holder', value: userEmail });
  }
...
export function generateStatementCanvas(statementData = {}) {
  ...
  const baseH = 500;
  const rowH = 36;
  const calculatedH = baseH + Math.max(1, rowsToDisplay.length) * rowH;
  ...
  if (rowsToDisplay.length === 0) {
    ctx.fillStyle = '#71717A';
    ctx.font = 'italic 500 12px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('No transactions recorded yet in this account', w / 2, curY + 16);
    curY += 36;
  } else {
    rowsToDisplay.forEach((tx, idx) => {
      ...
      ctx.fillText(truncateText(ctx, desc, 220), margin + 150, curY + 6);
...
/**
 * Download an HTMLCanvasElement directly as a PNG image with blob support for iOS/Android
 */
...
export async function downloadCanvasAsPdf(canvas, filename = 'asuk-receipt.pdf', title = 'Receipt') {
  ...
  doc.setProperties({
    title: title || 'Asuk Tech Receipt',
    subject: 'Official Wi-Fi Hotspot Receipt',
    author: 'Asuk Tech',
  });
...
export async function downloadStatementPdf(statementData = {}, filename = 'asuk-statement.pdf') {
  ...
  doc.setProperties({
    title: `${brand} - Account Statement`,
    subject: 'Financial Activity Ledger Statement',
    author: brand,
  });
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  doc.text(`Balance: ${formatPdfCurrency(statementData.walletBalance || 0)}`, 196, 30, { align: 'right' });
  ...
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text(`+${formatPdfCurrency(statementData.totalDeposits || 0)}`, 18, 72);
  ...
  doc.setFontSize(12);
  doc.text(`-${formatPdfCurrency(statementData.totalSpent || 0)}`, 80, 72);
  ...
  const tableData = transactions.length > 0 ? transactions.map((tx) => {
    const isDep = tx.type === 'wallet_topup' || tx.type === 'deposit';
    ...
    const amountStr = `${isDep ? '+' : '-'}${formatPdfCurrency(tx.amount)}`;
    ...
    ['-', 'No transaction records found in account', '-', '-', 'NGN 0.00', 'None']
  ];
  ...
  doc.autoTable({
    ...
    foot: [
      ['', '', '', 'Total Volume:', formatPdfCurrency(statementData.totalDeposits || 0), '']
    ],
    footStyles: {
...
/**
 * Native Print Utility: uses a hidden iframe to bypass popup blockers
 */
  ...
  try {
    ...
    frameDoc.write(`
      ...
      <html>
        ...
        <body>
          <img src="${dataUrl}" onload="setTimeout(function(){ window.focus(); window.print(); }, 150);" />
        </body>
      </html>
    `);
    frameDoc.close();
  } catch (err) {
    // Pop-up fallback if iframe access is blocked
