...
function drawVoucherCard(ctx, x, y, voucher, options = {}) {
  ...
  // WiFi icon indicator (simple text)
  ctx.font = '12px "Inter", "Segoe UI", system-ui, sans-serif';
  ctx.textAlign = 'left';
  ctx.fillText('Wi-Fi Voucher', x + 16, y + 40);
  ...
  if (voucher.expiry || voucher.plan_name) {
    ...
    if (showExpiry && voucher.expiry && voucher.plan_name) {
      ctx.font = '11px "Inter", "Segoe UI", system-ui, sans-serif';
      ctx.fillText(voucher.expiry, x + w - 16, y + 38);
    }
  }
  ...
  ctx.fillStyle = s.subColor;
  ctx.textAlign = 'center';
  ctx.fillText('Connect to Wi-Fi → Open browser → Enter code above → Enjoy!', x + w / 2, footerY);
